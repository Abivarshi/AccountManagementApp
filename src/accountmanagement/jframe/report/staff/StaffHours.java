 ...
 * @author acer
 */
public class StaffHours extends javax.swing.JPanel {
     ...
     * @param col
     */
    public StaffHours(String shopName, String title, String col) {
    ...
    private void searchButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_searchButtonActionPerformed
        ...
        if (fromDate.compareTo(toDate) < 0 || fromDate.compareTo(toDate) == 0) {
            ...
            Map<String, Map<String,Float[]>> dataVal = new HashMap();

            Map<String, String> staffList = getStaffList();
            ...
            try {
                ...
                for (String staff : staffList.keySet()) {
                    ResultSet res = db.getStaffTimeType(shopName, staff, fromDate, toDate, col);
                    while (res.next()) {
                        if (dataVal.containsKey(res.getString("Date"))) {
                            Map<String,Float[]> list = dataVal.get(res.getString("Date"));
                            Float[] val = new Float[3];
                            val[0] = res.getFloat("StartTime");
                            val[1] = res.getFloat("EndTime");
                            val[2] = res.getFloat("Hours");
                            list.put(staff, val);
                            dataVal.replace(res.getString("Date"), list);
                        } else {
                            ...
                            val[2] = res.getFloat("Hours");
                            list.put(staff, val);
                            dataVal.put(res.getString("Date"), list);
                        }
                    }
                }

                for (String date : dataVal.keySet()) {
                    Map<String, Float[]> list = dataVal.get(date);
                    Map<String, String> map = new HashMap();
                    map.put("Date", date);
                    map.put("title", title.toUpperCase());
                    map.put("description", description);
                    int j = 0;
                    for (String staff : list.keySet()) {
                        Float[] val = list.get(staff);
                        if (val[0] == null) {
                            map.put("Col" + (j + 1) + "_1", "");
                        } else {
                            map.put("Col" + (j + 1) + "_1", Float.toString(val[0]));
                        }
                        if (val[1] == null) {
                            map.put("Col" + (j + 1) + "_2", "");
                        } else {
                            map.put("Col" + (j + 1) + "_2", Float.toString(val[1]));
                        }
                        if (val[2] == null) {
                            map.put("Col" + (j + 1) + "_3", "");
                        } else {
                            map.put("Col" + (j + 1) + "_3", Float.toString(val[2]));
                        }
                        map.put("ColName" + (j + 1), staffList.get(staff));
                        j = j + 1;
                    }
                ...
                if (!data.isEmpty()) {
                    ...
                    InputStream sourceName = getClass().getResourceAsStream("/accountmanagement/jframe/report/StaffReport" + size + ".jrxml");
    ...
    private Map<String, String> getStaffList() {
        ...
        try {
            while (res.next()) {
                ...
                boolean hour = res.getBoolean(col);

                if (hour) {
                    staffList.put(staffColName, staffName);
                }
